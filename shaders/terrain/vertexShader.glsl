...
// For a fragment shader the variable is interpolated (the type of interpolation can be modified,
// try placing 'flat' in front here and in the fragment shader!).
out VertexData
{
    float v2f_height
...
void main()
{
    ...
    // We transform the position using one matrix multiply from model to clip space.
    // Note the added 1 at the end of the position to make the 3D coordinate homogeneous.
